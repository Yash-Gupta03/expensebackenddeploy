async function expensedata(e) {
  ...
  try{
  const response = await axios.post(`http://13.232.122.59:3000/expense/add-expense`, obj, {
...
async function getExpense(page){
  ...
  const limit = document.getElementById('pagesize').value;
    ...
    console.log(limit);
  const res = await axios.get(`http://13.232.122.59:3000/expense/get-expense?page=${page}&limit=${localStorage.getItem('rowLimit')}`, {
    headers: { Authorization: token },
  });
...
async function deleteExpense(id) {
  try{
  const token = localStorage.getItem("id");
  const res = await axios.delete(`http://13.232.122.59:3000/expense/delete-expense/${id}`, {
      headers: { Authorization: token },
    })
      removeFromScreen(id);
  }
...
document.getElementById("rzp-btn").onclick = async function (e) {
  const token = localStorage.getItem("id");
  const response = await axios.get(`http://13.232.122.59:3000/purchase/premiummembership`,
    {
      headers: {
        Authorization: token,
      },
  ...
  let options = {
    ...
    'handler': async function x(response) {
      ...
      const res = await axios.post(
        `http://13.232.122.59:3000/purchase/updatetransactionstatus`,
        {
...
document.getElementById('leader-board').onclick = async function(){
  try{
    const res = await axios.get(`http://13.232.122.59:3000/premium/leaderboard`);
...
async function download(){
  const token = localStorage.getItem("id");
  const response = await axios.get(`http://13.232.122.59:3000/expense/download`, { headers: {"Authorization" : token} });
      if(response.status === 200){
